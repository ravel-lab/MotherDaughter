...
dir=/local/groupshare/ravel/mfrance/MMOTH/01_raw/human_removed
...
#mkdir -p 02_fasta/
	...
	#### remove phiX reads ####
        #bowtie -p 16 -l 21 --mm /local/groupshare/ravel/BMA/1_db/phiX/phiX --suppress 1,2,3,4,5,6,7,8 $tmpdir/${sample}_1_tmp.fastq --un $tmpdir/${sample}_1_tmp2.fastq
        #bowtie -p 16 -l 21 --mm /local/groupshare/ravel/BMA/1_db/phiX/phiX --suppress 1,2,3,4,5,6,7,8 $tmpdir/${sample}_2_tmp.fastq --un $tmpdir/${sample}_2_tmp2.fastq
	#cat $tmpdir/${sample}_1_tmp2.fastq | /usr/local/bin/fastq-stats/fastq-stats > $tmpdir/$sample.phiX.stat 	
	### quality control #######
	###turns out trimmonic PE mode does not generate paired end reads in the same order, not even the same set of reads!!!!
	java -jar /home/mfrance/software/Trimmomatic-0.38/trimmomatic-0.38.jar PE -threads 8 -phred33 $tmpdir/$sample.reads_non_rRNA.R1.fq $tmpdir/$sample.reads_non_rRNA.R2.fq $pedir/${sample}_1_de_all.fastq $pedir/${sample}_unpaired.1.fastq $pedir/${sample}_2_de_all.fastq $pedir/${sample}_unpaired.2.fastq ILLUMINACLIP:/home/mfrance/software/Trimmomatic-0.38/adapters/TruSeq3-PE.fa:2:30:10 LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:$miniLen
	...
	sort -k1,1 $pedir/$sample.R1.tab -T /local/groupshare/ravel/temp | /home/mfrance/software/MiSeq16S/fq_splitlines.pl > $pedir/${sample}.R1.fq
	sort -k1,1 $pedir/$sample.R2.tab -T /local/groupshare/ravel/temp | /home/mfrance/software/MiSeq16S/fq_splitlines.pl > $pedir/${sample}.R2.fq	
