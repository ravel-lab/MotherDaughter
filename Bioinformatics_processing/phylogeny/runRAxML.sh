...
#$ -b y -l mem_free=64G -P jravel-lab -q threaded.q -pe thread 8 -N RAxML_MMOTH -j y -o /local/scratch/mfrance/logs/ -e /local/scratch/mfrance/logs/
...
#raxml --all --msa /local/scratch/mfrance/MMOTH/09_phylogeny/MMOTH_all.phy -msa-format PHYLIP --data-type AA -prefix MMOTH_mags --seed 2 --model /local/scratch/mfrance/MMOTH/09_phylogeny/MMOTH_partition.txt --tree pars{10},rand{10} -T 8 --bs-trees autoMRE 
/usr/local/packages/raxml-8.2.12/bin/raxmlHPC-PTHREADS -T 8 -f a -x 34567 -p 12349 -# autoMRE -q /local/scratch/mfrance/MMOTH/09_phylogeny/MMOTH_partition.txt -s /local/scratch/mfrance/MMOTH/09_phylogeny/MMOTH_all.phy -n MMOTH_mags
