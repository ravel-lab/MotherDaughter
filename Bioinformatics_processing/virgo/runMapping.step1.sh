...
awk -F"\t" '{a[$2]++;}END{ for (i in a) print i"\t"a[i]}' $output/$PREFIX.reads2ref | sort -t$'\t' -k2nr,2 | awk -F"\t" -v reads=$reads '$2 >= reads' | awk -F"\t" 'NR==FNR{a[$1]=$2;next} ($1 in a) {print $0"\t"a[$1]}' $dir_annot/0.len - > $output/$PREFIX.out
